...
module.exports.deleteSubSection = async (req, res) => {
  ...
  try {
    ...
    const Section = course.syallabus.find((s) => s.section_id === sectionId);

    if (Section) {
      return res.status(404).json({ message: 'Section not found' });
    }

    const subSection = Section.subSection.find((s) => s.sub_id === subSectionId);

    if (!subSection) {
      return res.status(404).json({ message: 'Sub-section not found' });
    }

    Section.subSection = Section.subSection.filter((s) => s.sub_id !== subSectionId);
    await course.save();
