...
module.exports.deleteSubSection = async (req, res) => {
  ...
  try {
    const UpdateCourse = await CourseModal.findById(id);

    UpdateCourse.syallabus.map((val) => {
      if (val.subSection.some((sub) => sub.id === sub_id)) {
        val.subSection = val.subSection.filter((sub) => sub.id !== sub_id);
      }
    });

    await CourseModal.findByIdAndUpdate(
      UpdateCourse._id,
      UpdateCourse,
      { new: true }
    ).then((result) => {
      res
        .status(200)
        .json({ message: "Subsection has been deleted successfully", result });
    });
  } catch (err) {
