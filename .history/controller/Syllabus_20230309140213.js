...
//   });
// };
app.post("/courses/:id/syllabus", async (req, res) => {
  ...
  try {
    ...
    // Add new syllabus section to the course syllabus array
    course.syllabus.push({ Section: section, subSection: subSection });
    await course.save();

    res.status(201).send({ message: "Syllabus section added successfully" });
  } catch (error) {
    console.error(error);
