...
const CourseModel = require("../modal/Course")
...
module.exports.getSingleInstructorByCourseId = async (req, res) => {
    try {
        const course = await CourseModel.findById({ _id: req.params.id });
        const instructorId = course.instructorId;
        const instructor = await InstructorModal.findById({ _id: instructorId });
        res.status(200).json({ data: instructor, message: "InstructorData  fetched" });
    } catch (err) {
        res.status(404).json({ messege: "heyu err", status: err.status });
    }
}
// post request 
